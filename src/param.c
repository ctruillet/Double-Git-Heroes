...
Param setLorenz(Param_Lorenz L, Param P){
	P = (Param)malloc(sizeof(Param_s)); //Attribution de la mémoire
	P->lorenz = (Param_Lorenz)L; //Remplissage de la mémoire
	return P;
}

Param setVanDerPol(Param_VanDerPol V, Param P){
	P = (Param)malloc(sizeof(Param_s)); //Attribution de la mémoire
	P->vanderpol = (Param_VanDerPol)V; //Remplissage de la mémoire
...
Param setRossler(Param_Rossler R, Param P){
	P = (Param)malloc(sizeof(Param_s)); //Attribution de la mémoire
	P->rossler = (Param_Rossler)R; //Remplissage de la mémoire
...
Param setParamLorenz(Param param, double B, double P, double S){
	...
	parametersL = (Param_Lorenz)malloc(sizeof(Param_Lorenz_s)); //Attribution de la mémoire
...
Param setParamVanDerPol(Param param, double K, double M, double B, double S, double P, double Q){
	...
	parametersV = (Param_VanDerPol)malloc(sizeof(Param_VanDerPol_s)); //Attribution de la mémoire
...
Param setParamRossler(Param param, double A, double B, double C){
	...
	parametersR = (Param_Rossler)malloc(sizeof(Param_Rossler_s));  //Attribution de la mémoire
...
Param choix_param(int mode, char * LOG){ 
    Param parameters;
    parameters = (Param)malloc(sizeof(Param_s)); //On donne de la mémoire à parameters

    int defaut = 1;   //Variable permettant de differencier le choix des parametrers par défaut ou non
    double B=0, P=0, S=0;
    double K=0, M=0, Q=0;
    double A=0, C=0;
